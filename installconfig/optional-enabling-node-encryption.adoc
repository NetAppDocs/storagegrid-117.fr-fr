...
IMPORTANT: Avant qu'une appliance sur laquelle le chiffrement des nœuds est activé ne soit connectée au système de gestion des clés (KMS), les données de l'appliance sont chiffrées avec une clé temporaire qui n'est pas sécurisée. L'appareil n'est pas protégé contre le retrait ou le vol tant que la clé n'est pas réglée sur une valeur fournie par le KMS.
