...
+
** La *période* pour la règle est *jour 0 - pour toujours*. *Jour 0* signifie que la règle est appliquée lorsque les données sont hiérarchisées depuis ONTAP. *Forever* signifie que StorageGRID ne supprimera pas les données qui ont été hiérarchisées de ONTAP à moins qu'il ne reçoive une demande de suppression de ONTAP.
...
.. Sélectionnez *Time Period* et *Storage Pool* pour consulter le diagramme de rétention.
