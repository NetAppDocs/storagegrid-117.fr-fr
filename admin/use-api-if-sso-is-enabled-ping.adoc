...
[role="lead"]
Si vous l'avez link:../admin/configuring-sso.html["Authentification unique (SSO) configurée et activée"] De plus, vous utilisez PingFederate comme fournisseur SSO, vous devez émettre une série de requêtes API pour obtenir un jeton d'authentification valide pour l'API Grid Management ou l'API tenant Management.
