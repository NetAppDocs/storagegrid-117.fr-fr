...
* Vous avez le link:admin-group-permissions.html["Autorisation d'accès racine ou configuration de la page de topologie de la grille et autres autorisations de configuration de la grille"].
...
Pour une explication détaillée de l'influence du paramètre Espace réservé aux métadonnées sur l'espace autorisé pour le stockage des métadonnées d'objet sur un nœud de stockage particulier, consultez link:managing-object-metadata-storage.html["Gérer le stockage des métadonnées d'objet"].
