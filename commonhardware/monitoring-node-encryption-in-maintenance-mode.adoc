...
Voir link:../admin/kms-configuring.html["Configurer les serveurs de gestion des clés"] Pour plus d'informations sur l'implémentation de KMS pour les appliances StorageGRID,
...
+
*** Pour résoudre les problèmes avec les certificats eux-mêmes, tels que le renouvellement des certificats expirés, consultez le link:../admin/kms-configuring.html["Instructions de configuration de KMS"].
...
link:../maintain/grid-node-decommissioning.html["Mise hors service du nœud"] Pour déplacer toutes les données qu'il contient vers d'autres nœuds de StorageGRID.
